void blinky_update(int blinky_loc[2], int pacman_loc[2], int map[NUM_TILES_Y][NUM_TILES_X], int pacman_mode, int ghost_mode)
{
    ...
    {
        distance_y = 1 - blinky_loc[1];// change
        distance_x = NUM_TILES_Y - 1 - blinky_loc[0];
    }
    else // chase mode
    {
    ...
    int moveright;
    int moveleft;
    if (map[blinky_loc[0] + 1][blinky_loc[1]] == 45 || map[blinky_loc[0] + 1][blinky_loc[1]] == 46 || map[blinky_loc[0] + 1][blinky_loc[1]] == 47)
    {
        moveright = 1;
    }
    else
    ...
    if (map[blinky_loc[0]][blinky_loc[1] - 1] == 45 || map[blinky_loc[0]][blinky_loc[1] - 1] == 46 || map[blinky_loc[0]][blinky_loc[1] - 1] == 47)
    {
        moveup = 1;
    }
    else
